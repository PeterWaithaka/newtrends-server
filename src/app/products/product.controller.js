...
var Model = require('./product.model.js');

/********************************************
******************** database calls **********/

function getAll() {
	return Model.find({}).populate('category').populate('subcategory').sort({ unit_price: -1 }).exec();
}
...
function find(query) {
	return Model.find(query).populate('category').populate('subcategory').sort({ unit_price: -1 }).exec();
}
